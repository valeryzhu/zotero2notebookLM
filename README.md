...
这是一个面向 Zotero 9 的项目骨架，目标是把本地目录中的 PDF 文献整理成 NotebookLM 友好的导入包，并为后续浏览器自动化上传预留接口。

当前设计不直接承诺调用 NotebookLM 官方 API，因为目前没有确认到公开、稳定的 NotebookLM 批量导入 API。第一版采用更稳的“两段式”流程：

1. Zotero 插件：在 Zotero 中提供入口，后续负责选择目录、扫描 PDF、导入到 collection、触发导出。
2. 本地 helper：把 PDF 复制到导入目录，生成 `manifest.json`、`manifest.csv` 和运行日志，后续可接 Playwright 上传器。

## 目录结构

- `zotero-plugin/`：Zotero 9 XPI 插件源码。
- `helper/`：本地 Node.js CLI helper。
...
- 已有 Zotero 插件骨架、菜单入口和偏好占位。
- 已有本地 helper，可扫描 PDF 并生成 NotebookLM 导入包。
- 尚未实现 Zotero 内部 PDF 导入、元数据抓取和 NotebookLM 网页自动化上传。
